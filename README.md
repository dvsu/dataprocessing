...
A simple processor class which downsamples large 2D array to smaller-sized array.  
The downsampling reduces the number of column and row in consistent step by calculating the mean/ average of target grid areas.  
...
- `convert_to_2x2(raw_data)`
- `convert_to_4x4(raw_data)`
- `convert_to_2x2(raw_data)`
- `convert_to_2x2(raw_data)`
- `convert_to_2x2(raw_data)`
- `grid_statistics(array)`
...
This processor is very handy to process array data of infrared array sensors, such as Panasonic Grid-EYE series and Omron D6T series

## Limitation

- Only work if `n_column` = `n_row`
- Fixed downsampling step, i.e. the raw data must has specific length

  ```none
  length = 2^(n+2) where n is an even number and n>=0, i.e. n=0,2,4,...
  ```

- Raw data must be in string, e.g. `"12.3,45.6,78.9,..."`

## Todo

- Able to accept multiple raw data type, especially a list/ array of number `[12.3,45.6,78.9,...]`
